...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint, whether_print=False, print_file_name=""):
    ...
    # xzjin add
    if whether_print:
        # d1 = "../h/spider_sample_v15_v2_complex.txt"
        details = open(print_file_name, mode='r', encoding="utf-8").readlines()
    # xzjin add
    debug = False
    if len(plist[0]) == 1:
        debug = True
            ...
            if etype in ["all", "exec"]:
                ...
                if current_os == 'Linux':
                    ...
                    try:
                        exec_score = eval_exec_match(db=db, p_str=p_str, g_str=g_str, plug_value=plug_value,
                                                     keep_distinct=keep_distinct, progress_bar_for_each_datapoint=progress_bar_for_each_datapoint, debug=debug)
                    except TimeoutError:
                        exec_score = 0
                    finally:
                        signal.alarm(0)

                elif "win" in current_os.lower():
                    exec_score = eval_exec_match(db=db, p_str=p_str, g_str=g_str, plug_value=plug_value,
                                                 keep_distinct=keep_distinct, progress_bar_for_each_datapoint=progress_bar_for_each_datapoint, debug=debug)
                ...
                if whether_print:
                    ...
                    with open(file_name, mode='a', encoding="utf-8") as f:
                        data = json.loads(details[my_idx])
                        for k, v in data.items():
                            ...
                            if k == "features":
                                f.write(f"-----------------------------------------------\n")
                            # f.write(f"{k}: {v}\n")
                            if k == "init_sql_results":
                                pred = v.get("sql", "")  # noqa
                            if k == "sql_results":
                                pred_correct = v.get("sql", "")  # noqa
...
if __name__ == "__main__":
    ...
    parser.add_argument('--whether_print', default=False, action='store_true', help='whether to print processed file')
    parser.add_argument('--print_file_name', type=str, help='the name of processed file')
    ...
    evaluate(args.gold, args.pred, args.db, args.etype, kmaps, args.plug_value,
             args.keep_distinct, args.progress_bar_for_each_datapoint, args.whether_print, args.print_file_name)
